...
final class ArrayShapeGenerator
{
    ...
    private static function doGenerateJetBrainsArrayShape(NodeInterface $node, int $nestingLevel = 1): string
    {
        ...
        $shape = '';
        if (1 === $nestingLevel && !$node->getParent() instanceof PrototypedArrayNode) {
            $shape = '#[ArrayShape(';
        } elseif (!$children) {
            return "'array<array-key, mixed>'";
        }
        ...
        if ($node instanceof PrototypedArrayNode) {
            $prototype = $node->getPrototype();
            if ($prototype instanceof ArrayNode) {
                return (1 === $nestingLevel ? $shape.'[' : '').self::doGenerateJetBrainsArrayShape($prototype, $nestingLevel).(1 === $nestingLevel ? '])]' : '');
            }

            return self::handleScalarNode($prototype);
        }
        ...
        foreach ($children as $child) {
            ...
            if ($child instanceof PrototypedArrayNode) {
                $shape .= '['.self::handleNode($child->getPrototype(), $nestingLevel, self::FORMAT_JETBRAINS_ATTRIBUTE).']';
            } else {
                $shape .= self::handleNode($child, $nestingLevel, self::FORMAT_JETBRAINS_ATTRIBUTE);
            }
        ...
        return $shape.(1 === $nestingLevel && !$node->getParent() instanceof PrototypedArrayNode ? ')]' : '');
    }
