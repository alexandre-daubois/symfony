    ...
    public function build(array $configurations): \Closure
    {
        ...
        if (!is_file($path)) {
            ...
            foreach ($configurations as $alias => $configuration) {
                ...
                $function->addParamPhpDoc($alias, ArrayShapeGenerator::generate($configuration->getConfigTreeBuilder()->buildTree(), ArrayShapeGenerator::FORMAT_PHPDOC));
                $function->addParamAttribute($alias, ArrayShapeGenerator::generate($configuration->getConfigTreeBuilder()->buildTree(), ArrayShapeGenerator::FORMAT_JETBRAINS_ATTRIBUTE));
            }
